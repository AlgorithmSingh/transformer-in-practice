...
const TEACHING = [
  {
    learn: 'This is one complete decoder-style forward pass with toy numbers.',
    explain: 'The weights are fake-small, but the operation order matches a real decoder: ids → embeddings → attention → FFN → logits → probabilities.',
    point: `m = ${CONFIG.m}, d_model = ${CONFIG.d_model}, vocab size = ${CONFIG.n_vocab}.`,
    confusion: 'Do not imply these are GPT weights. The goal is faithful flow, not real model behavior.',
    next: 'Before the math starts, clarify why generation reads the last input position.',
  },
  {
    learn: 'A decoder predicts from each position, but generation uses the last row.',
    explain: 'The prompt has three tokens, so the equivalent of the reference image’s p4 is p3 here.',
    point: '“the dog bit” → m = 3, so use the prediction after position 2.',
    confusion: 'p4 does not mean the fourth generated token. It means the prediction made from the fourth input position in that diagram.',
    next: 'Now convert the three tokens into vectors.',
  },
  {
    learn: 'Token ids select rows from the embedding matrix.',
    explain: 'The token “bit” starts as a mostly verb-like vector before attention gives it context.',
    point: `X_bit = [${t.X[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'Embeddings are not one-hot vectors. They are learned dense feature vectors.',
    next: 'This initial residual stream will be updated by attention and the FFN.',
  },
  {
    learn: 'A transformer block adds updates; it does not replace the residual stream.',
    explain: 'Attention writes C, the FFN writes M, and both are added back into the same shape stream.',
    point: `R1 = X + C, R2 = R1 + M, shape stays ${CONFIG.m}×${CONFIG.d_model}.`,
    confusion: 'Residual does not mean skipping the sublayer. It means keeping the old stream and adding the sublayer result.',
    next: 'Compute the attention ingredients: Q, K, and V.',
  },
  {
    learn: 'Q, K, and V are learned projections of X.',
    explain: 'Q asks what to look for, K says what can match, and V carries the content that can be copied forward.',
    point: `Q_bit = [${t.Q[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'Q/K/V are not attention weights yet. They are ingredients used to compute attention.',
    next: 'Show one row by hand so Q feels concrete.',
  },
  {
    learn: 'A single vector-matrix multiply creates Q for one token.',
    explain: 'The current token vector for bit is multiplied by W_Q, which was learned during training and is fixed at inference time.',
    point: `[${t.X[LAST].map(x => fmt(x)).join(', ')}] × W_Q → [${t.Q[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'The model is not recalculating W_Q from the prompt. It is applying already-learned weights.',
    next: 'Add position information with RoPE.',
  },
  {
    learn: 'RoPE rotates Q and K so their dot products become position-aware.',
    explain: 'Only Q and K rotate because they decide matching. V carries content, so V stays unchanged.',
    point: 'Position 0 does not rotate; later positions rotate more.',
    confusion: 'RoPE is not the attention dot product. It is a preparation step before dot products.',
    next: 'Do the bit rotation explicitly.',
  },
  {
    learn: 'Position 2 changes the bit query coordinates by rotation.',
    explain: 'The same query now has position baked into its numbers, so matching can depend on relative position.',
    point: `Q_bit [${t.Q[LAST].map(x => fmt(x)).join(', ')}] → Q′_bit [${t.Qr[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'V is not rotated; only Q and K are.',
    next: 'Compare rotated queries to rotated keys.',
  },
  {
    learn: 'Attention scores are Q′·K′ comparisons.',
    explain: 'Each row asks: how much does this token’s query match each key?',
    point: `Last row scores = [${t.scores[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'Scores are raw similarities, not probabilities.',
    next: 'Before softmax, causal masking removes future positions.',
  },
  {
    learn: 'Decoder-only models cannot look ahead.',
    explain: 'Future scores become −∞ before softmax, so their probability becomes exactly zero.',
    point: 'First row future entries are −∞; last row is unchanged because it has no future prompt token.',
    confusion: 'Masking is per query row, not a global deletion of tokens.',
    next: 'Softmax turns the remaining scores into attention weights.',
  },
  {
    learn: 'Softmax converts scores into weights that sum to 1.',
    explain: 'For the bit row, attention spreads across all three prompt tokens.',
    point: `bit attends ${pct(t.A[LAST][0])} to the, ${pct(t.A[LAST][1])} to dog, ${pct(t.A[LAST][2])} to bit.`,
    confusion: 'Attention weights are not the content itself. They only say how much content to read.',
    next: 'Use these weights to blend V.',
  },
  {
    learn: 'Attention weights decide how much V content is collected.',
    explain: 'C_bit is a weighted blend of the value vectors from the, dog, and bit.',
    point: `C_bit = [${t.C[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'The model multiplies A by V, not by Q or K. Q/K decide relevance; V carries information.',
    next: 'Add this context back into the residual stream.',
  },
  {
    learn: 'Attention context is added to the original token representation.',
    explain: 'The bit vector keeps its original verb signal and gains context from the dog.',
    point: `[${t.X[LAST].map(x => fmt(x)).join(', ')}] + [${t.C[LAST].map(x => fmt(x)).join(', ')}] = [${t.R1[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'Attention does not overwrite the token vector.',
    next: 'The FFN now transforms this contextual vector.',
  },
  {
    learn: 'The FFN processes each position independently after attention.',
    explain: 'Attention mixed tokens; the FFN reshapes each already-contextual row with a nonlinear feature transformation.',
    point: `Hraw_bit second value ${fmt(t.Hraw[LAST][1])} → ${fmt(t.H[LAST][1])} after ReLU; M_bit = [${t.M[LAST].map(x => fmt(x)).join(', ')}].`,
    confusion: 'The FFN does not look at other tokens. Cross-token mixing happened in attention.',
    next: 'Add the FFN update back into the stream.',
  },
  {
    learn: 'R2 is the final output of this transformer block.',
    explain: 'After the second residual add, the last row is the hidden vector representing the whole prefix.',
    point: `h_final = [${t.hFinal.map(x => fmt(x)).join(', ')}].`,
    confusion: 'This is still not a probability. It is a hidden vector.',
    next: 'Unembedding converts this vector into vocabulary scores.',
  },
  {
    learn: 'Unembedding maps the hidden vector to one raw score per token.',
    explain: 'Each logit is a dot product between h_final and one output-token row of U.',
    point: `the=${fmt(t.logits[1])}, ran=${fmt(t.logits[6])}, bit=${fmt(t.logits[3])}.`,
    confusion: 'Logits are not percentages. They are unnormalized scores.',
    next: 'Softmax turns logits into next-token probabilities.',
  },
  {
    learn: 'Softmax gives a probability distribution over the vocabulary.',
    explain: 'The top token is “the”, but the toy model is not 100% certain.',
    point: `p(the) = ${pct(t.probs[1])}.`,
    confusion: 'The chosen token is toy-model behavior, not a claim that real GPT must choose the same word.',
    next: 'Recap the full causal chain.',
  },
  {
    learn: 'Q/K create the attention pattern; V carries content; residuals accumulate updates.',
    explain: 'Ask learners to narrate the path from ids to probability using the flow on screen.',
    point: 'Q·K → scores → mask → softmax → A·V → C → R2 → p(next).',
    confusion: 'Do not memorize matrix names without knowing each role.',
    next: 'Use this as the review slide before looking back at the original three diagrams.',
  },
];

function teachingBlock(teaching, originalNote) {
  return `<div class="speaker teaching">
    <p><b>What to learn:</b> ${teaching.learn}</p>
    <p><b>Speaker explanation:</b> ${teaching.explain}</p>
    <p><b>Key number to point at:</b> ${teaching.point}</p>
    <p><b>Common confusion to avoid:</b> ${teaching.confusion}</p>
    <p><b>Connects next:</b> ${teaching.next}</p>
    <p class="original-note"><b>Original speaker note:</b> ${originalNote}</p>
  </div>`;
}
...
deck.innerHTML = slides.map((s, i) => `
  <section class="slide" data-i="${i}" aria-label="Slide ${i + 1}: ${esc(s.title)}">
    ...
    ${teachingBlock(TEACHING[i], s.notes)}
  </section>`).join('');
